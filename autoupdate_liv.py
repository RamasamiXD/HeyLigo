...
url = "https://alog.ligo-la.caltech.edu/aLOG/iframeSrc.php?authExpired=&content=1&step=&callRep=&startPage=%s&preview=&printCall=&callUser=&addCommentTo=&callHelp=&callFileType=#"
    ...
    def extract_comments(coms,pid):
        ...
        if coms:
            ...
            for i in ids:
                ...
                comment['author_id']= textify(temp[0][:temp[0].find('@')])
                comment['comment_time'] = dt.datetime.strptime(" ".join(temp[2:7]),'%H:%M, %A %d %B %Y')
                comment['content'] = textify(" ".join(content[1].text.split()))
                if len(content)>2:
...
def updateCSV():
    global reports
    pro_df = pd.read_csv('Livingston/Original_ProFreports.csv',index_col=0,encoding='utf-8')
    comm_df = pd.read_csv('Livingston/Original_FComments.csv',index_col=0,encoding='utf-8')
    ...
    for page in range(1,10000):
        ...
        try:
            ...
            break
        soup = bs(r.text,"html.parser")
        if "No reports meet the specified criteria." in soup.text:
            break
        l = soup.findAll("div",id=re.compile('^report_'))
        for i in l:
            extract_info(i)
    ...
    pro_df.to_csv('Livingston/Original_ProFreports.csv',encoding='utf-8')
    ...
    comm_df.to_csv('Livingston/Original_FComments.csv',encoding='utf-8')
...
def createGensim():
    pro_df = pd.read_csv('Livingston/Original_ProFreports.csv',index_col=0,encoding = 'utf8')
    pro_df = pro_df.replace(np.nan, '', regex=True)
    sentences = []
    for line in pro_df['title_authors_stems']:
    ...
    model_name = "Livingston/all_titles_authors_model"
    ...
    joblib.dump(normalised,'Livingston/normalised.pkl')
    joblib.dump(title_author_mat,'Livingston/title_author_mat.pkl')
...
def createTFIDF():
    ...
    joblib.dump(words,'Livingston/words.pkl')
    joblib.dump(idf,'Livingston/idf.pkl')
    joblib.dump(norm,'Livingston/norm.pkl')
    ...
    word_to_wordID_file = open("Livingston/Livingston_word_to_wordID_file", "wb")
    pickle.dump(word_to_wordID, word_to_wordID_file)

    documents_to_concepts_file = open("Livingston/Livingston_documents_to_concepts_file", "wb")
    pickle.dump(documents_to_concepts, documents_to_concepts_file)

    concepts_file = open("Livingston/Livingston_concepts_file", "wb")
    pickle.dump(concepts, concepts_file)

    concepts_to_words_file = open("Livingston/Livingston_concepts_to_words_file", "wb")
...
def check_comments():
    fdata = pd.read_csv('Livingston/Original_FComments.csv',encoding = "utf8")
    clist = []
...
def check_code_link():
    fname = 'Livingston/ProFreports.csv'
    os.remove(fname) if os.path.exists(fname) else None
    data = pd.read_csv('Livingston/Original_ProFreports.csv',encoding = "utf8")
    ...
    if os.path.exists('Livingston/Original_FComments.csv'):
